...
![Mala Redis knjiga, Karl Seguin](title.png)\
...
Mala Redis knjiga je licencirana pod Attribution-NonCommercial 3.0 Unported licencom. Ova knjiga nije za prodaju.

Slobodni ste da kopirate, distribuirate, menjate ili prikazujete knjigu. Međutim, molim vas da mene, Karla Seguina, naznačite kao autora knjige i da je ne koristite u komercijalne svrhe.

**Ceo tekst** licence možete naći ovde:

<http://creativecommons.org/licenses/by-nc/3.0/legalcode>

### O autoru

Karl Seguin je programer sa iskustvom u raznim poljima i tehnologijama. Aktivan je kontributor projektima otvorenog izvornog koda, tehnički pisac i povremeni govornik. Napisao je razne članke, kao i nekoliko alata, o Redisu. Redis koristi za rangiranje i statistiku svog slobodnog servisa za neprofesionalne programere igrica: [mogade.com](http://mogade.com/).

Karl je napisao [Malu MongoDB knjigu](http://openmymind.net/2011/3/28/The-Little-MongoDB-Book/), besplatnu i popularnu knjigu o MongoDB-u.

Njegov blog možete naći na <http://openmymind.net>, a njegove tvitove ovde [@karlseguin](http://twitter.com/karlseguin)

### Zahvalnice

Posebno se zahvaljujem [Periju Nilu](https://twitter.com/perryneal) na pozajmici svojih očiju, uma i strasti. Pružio si mi neprocenjivu pomoć. Hvala.

### Poslednja verzija

Poslednji izvorni kod ove knjig se nalazi na:
<http://github.com/karlseguin/the-little-redis-book>

### O prevodiocu

[Nikola Kotur](https://twitter.com/kotnik) je programer koji voli da prevodi tehničke članke.

\clearpage

## Uvod

Poslednjih nekoliko godina, tehnike i alati za čuvanje i upit nad podacima su narasli neverovatnom brzinom. Iako je sigurno da relacione baze podataka neće nestati, takođe je tačno da ekosistem oko podataka nikada više neće biti isti.

Od svih novih alata i rešenja, meni je Redis bio najuzbudljiviji. Zašto? Prvo, zato što se neverovatno lako uči. Vreme potrebno da se osećate udobno sa Redisom se meri u satima. Drugo, rešava specifičan set problema i u isto vreme ostaje prilično generičan. Šta to znači? Redis ne pokušava da bude sve za svaku vrstu podataka. Kako budete upoznavali Redis, postaće očigledno čemu jeste, a čemu nije mesto u njemu. A kada jeste, za developera, uživanje je raditi sa njime.

Iako možete napraviti kompletan sistem koristeći samo Redis, mislim da će većina ljudi uvideti da Redis samo doprinosi njihovom uopštenom rešenju za podatke - bilo da je u pitanju tradicionalna relaciona baza podataka, sistem orjentisan ka dokumentima, ili nešto treće. U pitanje je rešenje koje koristite da implementirate specifične osobine. U tom smislu, sličan je sistemu za indeksiranje. Nećete napraviti kompletnu aplikaciju baziranu na Lucene. Ali, kada vam je potrebna dobra pretraga onda Lucene pruža mnogo bolje iskustvo - i vama i vašim korisnicima. Naravno, sličnosti između Redisa i sistema za indeksiranje se tu završavaju.

Cilj ove knjige je da napravi osnovu koja vam je potrebna kako biste ovladali Redisom. Fokusiraćemo se na učenje Redisovih pet struktura podataka i obratiti pažnju na razne pristupe modeliranju podataka. Takođe ćemo dodirnuti neke ključne administrativne detalje i tehnike za traženje grešaka.

## Početak

Svi mi učimo na različite načine: neko voli da odmah prione na posao, neko voli da odgleda video uputstva, dok neko voli da čita. Ništa vam neće pomoći da razumete Redis od stvarnog iskustva. Redis je lako instalirati i sa njim dolazi jednostavni komandni prompt koji pruža sve što je potrebno. Sada ćemo odvojiti par minuta i podesiti naše računare.

### Na Windowsu

Redis sam po sebi zvanično ne podržava Windows, ali opcije postoje. Ne biste ih koristili za produkcione sisteme, ali ja nikad nisam naišao na neko ograničenje u toku razvoja aplikacija.

Port od strane Microsoft Open Technologies, Inc. možete naći na <https://github.com/MSOpenTech/redis>. U trenutku pisanja ovo rešenje nije spremno za produkcione sisteme.

Još jedno rešenje, koje postoji već dugo vremena, možete naći na <https://github.com/dmajkic/redis/downloads>. Možete preuzite najnoviju verziju (koja bi trebala biti na vrhu liste). Raspakujte zip fajl i, u zavisnosti od vaše arhitekture, otvorite bilo `64bit` ili `32bit` folder.

### Na *nix i MacOSX-u

Najbolja opcija za *nix and and Mac korisnike je kompajliranje izvornog koda. Instrukcije, zajedno sa brojem poslednje verzije, se nalaze na <http://redis.io/download>. U trenutku pisanja poslednja verzija je 2.4.6; da biste instalirali ovu verziju treba da uradite:
...
(Alternativno, Redis je dostupan preo raznih menadžera paketa. Na primer MacOSX korisnici sa instaliranim Homebrew programom jednostavno mogu da ukucaju `brew install redis`.)
